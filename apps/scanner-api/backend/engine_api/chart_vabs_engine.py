"""
chart_vabs_engine.py — Phase 8F-A: port of root backend/vabs_engine.py.

Volume Absorption & Breakout signals for Super Chart History timeline.
No yfinance. No imports from old root backend.
Verbatim formula port — only import path changed.
"""
...
from .chart_indicators import atr as _atr_hlc, rma
...
def compute_vabs(df: pd.DataFrame) -> pd.DataFrame:
    ...
    climb_sig = (bkt > prev_bkt) & (prev_bkt > prev2_bkt) & (bkt >= _CLIMB_MIN_END)
    ...
    vol_score     = (ratio_ok.astype(int) + ma_ok.astype(int) +
    ...
    # Vol spike multiplier badges (V×5 / V×10 / V×20)
    vol_spike_5x  = v >= vol_ma * 5.0
    vol_spike_10x = v >= vol_ma * 10.0
    vol_spike_20x = v >= vol_ma * 20.0
    ...
    return pd.DataFrame({
        ...
        "vol_spike_10x": vol_spike_10x,
        "vol_spike_20x": vol_spike_20x,
    }, index=df.index)


# Display label order for per-bar active-signal extraction
VABS_SIG_COLS: list[tuple[str, str]] = [
    ("vol_spike_20x", "V×20"),
    ("vol_spike_10x", "V×10"),
    ("vol_spike_5x",  "V×5"),
    ("best_sig",      "BEST★"),
