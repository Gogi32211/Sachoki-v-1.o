"""
chart_ultra_engine.py — Phase 8G commit 7: port of root backend/ultra_engine.py.

260308 + L88 + ULTRA v2 (EB / FBO / BF / VSA / 3up).
Verbatim formula port — only the indicator + wlnbb import paths are changed.

compute_260308_l88(df)  → sig_260308, sig_l88
compute_ultra_v2(df)    → eb_bull/bear, fbo_bull/bear, bf_buy/sell, ultra_sq/ns/nd,
                         ultra_3up/3dn, best_long, best_short
"""
...
from .chart_indicators import atr as _atr_hlc
...
def compute_260308_l88(
    ...
    try:
        from .chart_wlnbb_engine import compute_wlnbb
        wl   = compute_wlnbb(df)
...
# Display routing — ULT row
ULT_SIG_COLS: list[tuple[str, str]] = [
    ("sig_l88",    "L88"),
    ("sig_260308", "260308"),
    ("best_long",  "BEST↑"),
    ("best_short", "BEST↓"),
